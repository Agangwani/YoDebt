...
// send post request to firestore -- use firebase for authentication portion of this
// use java code as bridge between front end and storage

// front end people talk to firebase and then it comes back authenticated
...
public class Mortgage {
    ...
    public static ArrayList<ArrayList<Float>> showValues(Mortgage mortgage, String[] underwriteRate, boolean payThroughSchool, float setSchoolPay){
        float loanAmount = mortgage.loanAmount;
        float termMonths = mortgage.termmonths;
...
// if you're reading this you fell asleep so just go and make the module to assume that the
                // monthly downpayments are equal to 0 for the first 54 months 48 for 4 years college
                // and 6 months for grace period provided
            }
            if(payThroughSchool== false && x<= 54){
                monthlyPay = setSchoolPay;
            }
            Month.add((float)x);
            LoanAfterMonths.add(newLoanAmt);
            float IntPayment = newLoanAmt*couponValue/12;
    ...
    public static void main(String[] args) {
        ...
        ArrayList<Float> YearlyPayment = new ArrayList<>();
        for(Float element : monthlyPayments) YearlyPayment.add(element * 12);
        ...
        System.out.println("YEARLY PAYMENTS " + YearlyPayment + "\n" );
